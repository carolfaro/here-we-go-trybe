...
const somarPares = (acumulador, number) => {
    if (number % 2 === 0) { // number são todos os valores do array
    return acumulador + number; // entao soma
    } 
    return acumulador; // senao retorna o acumulador 
    
};;

const resultado = numbers.reduce(somarPares);
...
const estudantes = [
  ...
  const verificaNota = (acumulador, materias) => {
      if (materias > acumulador) {
          return materias;
      }
      return acumulador;
  };
